 ...
 * 
 * The inspiration for this implementation came from http://www.DaveKoelle.com
 * However, his implementation did not handle leading 0's properly, nor did it handle
 * nulls or case sensitivity.
 * 
...
public class AlphanumComparator implements Comparator<String>
{
	...
	/**
	 * Get a reference to a cached, shared instance. Good for reuse, but would have multithreading
	 * issues if many threads are trying to sort at the same time.
	 * 
	 * @param caseSensitive
	 */
	...
	public int compare(String s1, String s2)
	{
		...
		while (thisMarker < s1Length && thatMarker < s2Length)
		{
			...
			if (isDigit(thisChunk.charAt(0)) && isDigit(thatChunk.charAt(0)))
			{
				...
				else if (thisChunkInt.length < thatChunkInt.length)
				{
				...
				for (int i = 0; i < thisChunkInt.length; i++)
				{
					...
					else if (thisChunkInt[i] < thatChunkInt[i])
					{
						result = -1;
						break;
					}
